...
{	
	...
	SOCKET connSocket = connect(serverSocket, (SOCKADDR*)&addrSrv, sizeof(SOCKADDR));
	...
	//从数值（addr）转换到表达式(str)
	PCSTR ntop = inet_ntop(AF_INET, &clientAddr.sin_addr, str, sizeof(str));
	
	sprintf_s(sendBuf, message, info->wxid, info->wxcount,info->nickname,info->wxsex,info->phone,
		info->device,info->nation,info->province,info->city,info->header);
